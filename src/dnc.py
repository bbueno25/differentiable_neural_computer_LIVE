...
class DNC:
    ...
    def run(self):
        '''
        output list of numbers (one hot encoded)
        by running the step function
        '''
        big_out = []
    ...
    def step_m(self, x):
        ...
        partition = (tf.constant([[0] * (self.num_heads  * self.word_size) +
                                  [1] * (self.num_heads) +
                                  [2] * (self.word_size) +
                                  [3] +
                                  [4] * (self.word_size) +
                                  ...
                                  [7] +
                                  [8] +
                                  [9] * (self.num_heads * 3)], dtype=tf.int32))
        ...
        self.mem_mat = (self.mem_mat *
                       (1 - tf.matmul(self.write_weights, erase_vec)) +
...
def main(argv=None):
    '''
    generate the input output sequences,
    randomly intialized
    '''
    num_seq = 10
    seq_len = 6
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            regularizers = (tf.nn.l2_loss(dnc.W1) + tf.nn.l2_loss(dnc.W2) + tf.nn.l2_loss(dnc.b1) + tf.nn.l2_loss(dnc.b2))
            # to help the loss convergence faster
            loss += 5e-4 * regularizers
            # optimize the entire thing (memory + controller) using gradient descent
